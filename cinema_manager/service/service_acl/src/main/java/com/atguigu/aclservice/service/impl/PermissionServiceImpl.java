...
//        });
    ...
    public void saveRolePermissionRealtionShipGuli(String roleId, String[] permissionIds) {
        ...
        //1 创建set集合，用于封装添加数据
        HashSet<RolePermission> newRolePermissionSet = new HashSet<>();
        //遍历所有菜单数组
        for(String perId : permissionIds) {
            //得到父id的集合
            List<String> pIds=getParentIds(perId);
            //将所有父权限也添加进去
            for (String pId : pIds) {
                RolePermission rolePermission = new RolePermission();
                rolePermission.setRoleId(roleId);
                rolePermission.setPermissionId(pId);
                newRolePermissionSet.add(rolePermission);
            }
            //RolePermission对象
            RolePermission rolePermission = new RolePermission();
            rolePermission.setRoleId(roleId);
        ...
        //将最终的set转换为list
        List<RolePermission> newRolePermissionList = new ArrayList<>(newRolePermissionSet);
    ...
    //递归获得所有父id
    private List<String> getParentIds(String perId) {
        List<String> finalList = new ArrayList<>();
        getParentId(perId,finalList);
        return finalList;
    }
    private void getParentId(String perId,List<String> finalList){
        Permission permission = baseMapper.selectById(perId);
        if(permission!=null){
            String pid =permission.getPid();//得到父id
            finalList.add(pid);//把父id加到最终的集合中
            getParentId(pid,finalList);
        }
    }
}
