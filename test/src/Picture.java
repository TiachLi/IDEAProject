...
public class Picture {
    public static void main(String[] args) throws Exception{
        //指定的图片路径
        FileInputStream dir = new FileInputStream("D:/证件照.jpg");
        ...
        for (int i = minx; i < width; i++) {
            for (int j = miny; j < height; j++) {
                ...
                if (rgb[0]<120&&rgb[0]>=0&&rgb[1]<180&&rgb[1]>130 && rgb[2]<210&&rgb[2]>90) {        //修改像素点，0x007ABB是证件照的蓝色背景色
                    bImage.setRGB(i, j, 0xFFFF00);
                }
            }
        }
        //输出照片保存在本地
        ...
        try {
            ops = new FileOutputStream(new File("D:/2.jpg"));
