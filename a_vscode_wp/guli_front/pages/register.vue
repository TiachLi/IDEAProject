      ...
      <el-form ref="userForm" :model="params">
        ...
        <el-form-item class="input-prepend restyle no-radius" prop="code" :rules="[{ required: true, message: '请输入验证码', trigger: 'blur' }]">
          ...
          <div class="btn" style="position:absolute;right: 0;top: 6px;width: 40%;">
            <a href="javascript:" type="button" @click="getCodeFun()" :value="codeTest" style="border: none;background-color: none;text-decoration: none;">{{codeTest}}</a>
          </div>
        </el-form-item>
    ...
    methods: {
       ...
       submitRegister() {
         registerApi.registerMember(this.params)
          .then(response => {
          if(response.data.code===20001){//如果验证码错误，弹窗不关闭
            this.$message({
